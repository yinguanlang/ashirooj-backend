...
import com.ashiro.ashirooj.model.vo.ManagerQuestionVO;
...
public class QuestionController {
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        ...
        List<CodeTemplate> codeTemplate = questionAddRequest.getCodeTemplate();
        question.setCodeTemplate(GSON.toJson(codeTemplate));

        List<CodeTemplate> mainCode = questionAddRequest.getMainCode();
        question.setMainCode(GSON.toJson(mainCode));

        MetaData metaData = questionAddRequest.getMetaData();
        question.setMetaData(GSON.toJson(metaData));
    ...
    public BaseResponse<Boolean> deleteQuestion(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 删除该题目的缓存
        try {
            redisTemplate.delete(RedisConstant.QUESTION_ID_KEY+id);
        } catch (Exception e) {
    ...
    public BaseResponse<Boolean> updateQuestion(@RequestBody QuestionUpdateRequest questionUpdateRequest) {
        ...
        Question oldQuestion = questionService.getById(id);
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        List<CodeTemplate> codeTemplate = questionUpdateRequest.getCodeTemplate();
        question.setCodeTemplate(GSON.toJson(codeTemplate));

        List<CodeTemplate> mainCode = questionUpdateRequest.getMainCode();
        question.setMainCode(GSON.toJson(mainCode));

        MetaData metaData = questionUpdateRequest.getMetaData();
        question.setMetaData(GSON.toJson(metaData));
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Page<ManagerQuestionVO>> listQuestionByPage(@RequestBody QuestionQueryRequest questionQueryRequest) {
        long current = questionQueryRequest.getCurrent();
        long size = questionQueryRequest.getPageSize();
        Page<Question> questionPage = questionService.page(new Page<>(current, size),
                questionService.getQueryWrapper(questionQueryRequest));
        Page<ManagerQuestionVO> managerQuestionVOPage = new Page<>(current, size, questionPage.getTotal());
        ...
        List<ManagerQuestionVO> managerQuestionVOS = records.stream().map(question -> {
            ManagerQuestionVO questionVO = ManagerQuestionVO.objToVo(question);
            questionVO.setCodeTemplate(GSON.fromJson(question.getCodeTemplate(), new TypeToken<List<CodeTemplate>>() {
            }.getType()));
        ...
        managerQuestionVOPage.setRecords(managerQuestionVOS);
        return ResultUtils.success(managerQuestionVOPage);
    }
    ...
    public BaseResponse<QuestionSubmitResultVO> doQuestionSubmit(@RequestBody QuestionSubmitAddRequest questionSubmitAddRequest,
                                                                 HttpServletRequest request) {
        ...
        return questionSubmitService.doQuestionSubmit(questionSubmitAddRequest, loginUser);
    }
     ...
     * @return
     */
    @PostMapping("/question_submit/list/page/current_user")
    public BaseResponse<Page<QuestionSubmitVO>> listCurrentUserQuestionSubmitByPage(@RequestBody QuestionSubmitQueryRequest questionSubmitQueryRequest,
                                                                         HttpServletRequest request) {
        long current = questionSubmitQueryRequest.getCurrent();
        long size = questionSubmitQueryRequest.getPageSize();
        // 从数据库中查询原始的题目提交分页信息,传userId
        final User loginUser = userService.getLoginUser(request);
